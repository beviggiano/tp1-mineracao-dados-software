...
def test_cli_analyze_command(test_repo):
    result = runner.invoke(app, ["analyze", test_repo], mix_stderr=True)
    assert result.exit_code == 0
    assert "Análise" in result.stdout or "Iniciando" in result.stdout


def test_cli_security_command(test_repo):
    result = runner.invoke(app, ["security", test_repo], mix_stderr=True)
    assert result.exit_code == 0
